...
for k, traj in enumerate(example_trajs):
    ...
    plt.plot(pred['q_pred'], pred['p_pred'], '--', lw=2, label=f'Traj {traj} SympNet' if k==0 else None, color=f'C{k}', alpha=0.7)
plt.xlabel("q (angle)")
plt.ylabel("p (momentum)")
plt.title("Phase Space: True vs. SympNet Prediction\n(Shaded = Training region)")
plt.legend()
plt.savefig('Results/Pendulum_SYMPNET/phase_space.png', dpi=150)
plt.close()
...
for k, traj in enumerate(example_trajs):
    ...
    plt.plot(t, total_h(q_pred, p_pred), label="SympNet H", color='C0', alpha=0.5)
    plt.plot(t, kinetic(p_pred), label='SympNet K', ls='--', color='C1', alpha=0.5)
    plt.plot(t, potential(q_pred), label='SympNet V', ls='-', color='C2', alpha=0.5)
    plt.axvline(t_quarter, color='k', ls=':', label='1/4 period (end of train)', lw=2)
    plt.title(f"Energy Conservation: Traj {traj} (SympNet)")
    plt.xlabel("Time (s)")
    plt.ylabel("Energy")
    plt.legend()
    plt.savefig(f'Results/Pendulum_SYMPNET/energy_traj_{traj}.png', dpi=150)
    plt.close()
...
if mae_by_time:
    ...
    plt.plot(mae_by_time[:,0], mae_by_time[:,1], label="SympNet MAE (test)")
    plt.axvline(qpr_df['t'].max(), color='k', ls=':', label='Train/test split (max 1/4 T)')
    plt.xlabel("Time (s)")
    plt.ylabel("Mean Abs Error (q,p)")
    plt.title("SympNet Mean Absolute Error vs. Time (test region)")
    plt.legend()
    plt.savefig('Results/Pendulum_SYMPNET/mae_vs_time.png', dpi=150)
    plt.close()

print("Phase space plot: Results/Pendulum_SYMPNET/phase_space.png")
print(f"Energy plots: Results/Pendulum_SYMPNET/energy_traj_[traj].png (example: {example_trajs.tolist()})")
print("MAE curve: Results/Pendulum_SYMPNET/mae_vs_time.png")