...
os.makedirs('Results/Pendulum_GHNN', exist_ok=True)

full_df = pd.read_hdf('Data/Pendulum_MLP/pendulum_full.h5', key='trajectories')
pred_df = pd.read_hdf('NeuralNets/Pendulum_GHNN/ghnn_predictions.h5', key='preds')
...
for k, traj in enumerate(example_trajs):
    ...
    plt.plot(pred['q_pred'], pred['p_pred'], '--', lw=2, label=f'Traj {traj} GHNN' if k==0 else None, color=f'C{k}', alpha=0.7)
plt.xlabel("q (angle)")
plt.ylabel("p (momentum)")
plt.title("Phase Space: True vs. GHNN Prediction\n(Shaded = Training region)")
plt.legend()
plt.savefig('Results/Pendulum_GHNN/phase_space.png', dpi=150)
plt.close()
...
for k, traj in enumerate(example_trajs):
    ...
    plt.plot(t, total_h(q_pred, p_pred), label="GHNN H", color='C0', alpha=0.5)
    plt.plot(t, kinetic(p_pred), label='GHNN K', ls='--', color='C1', alpha=0.5)
    plt.plot(t, potential(q_pred), label='GHNN V', ls='-', color='C2', alpha=0.5)
    plt.axvline(t_quarter, color='k', ls=':', label='1/4 period (end of train)', lw=2)
    plt.title(f"Energy Conservation: Traj {traj} (GHNN)")
    plt.xlabel("Time (s)")
    plt.ylabel("Energy")
    plt.legend()
    plt.savefig(f'Results/Pendulum_GHNN/energy_traj_{traj}.png', dpi=150)
    plt.close()
...
if mae_by_time:
    ...
    plt.plot(mae_by_time[:,0], mae_by_time[:,1], label="GHNN MAE (test)")
    plt.axvline(qpr_df['t'].max(), color='k', ls=':', label='Train/test split (max 1/4 T)')
    plt.xlabel("Time (s)")
    plt.ylabel("Mean Abs Error (q,p)")
    plt.title("GHNN Mean Absolute Error vs. Time (test region)")
    plt.legend()
    plt.savefig('Results/Pendulum_GHNN/mae_vs_time.png', dpi=150)
    plt.close()

print("Phase space plot: Results/Pendulum_GHNN/phase_space.png")
print(f"Energy plots: Results/Pendulum_GHNN/energy_traj_[traj].png (example: {example_trajs.tolist()})")
print("MAE curve: Results/Pendulum_GHNN/mae_vs_time.png")